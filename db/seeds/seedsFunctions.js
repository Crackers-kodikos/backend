...
export const seedDatabase = async () => {
    try {
        ...
        const seedResult = await db.transaction(async (tx) => {
            ...
            const plansResult = await tx.insert(subscriptionPlans)
                .values(plansData)
                .returning();
            ...
            const workshopOwnerHash = await hashPassword(workshopOwnerPassword);
            ...
            const workshopOwnerResult = await tx.insert(users)
            ...
            const validatorHash = await hashPassword(validatorPassword);
            ...
            const validatorUserResult = await tx.insert(users)
            ...
            const tailorHash = await hashPassword(tailorPassword);
            ...
            const tailorUserResult = await tx.insert(users)
            ...
            const tailor2UserResult = await tx.insert(users)
            ...
            const magazineHash = await hashPassword(magazinePassword);
            ...
            const magazineUserResult = await tx.insert(users)
            ...
            const magazine2UserResult = await tx.insert(users)
            ...
            const workshopResult = await tx.insert(workshops)
                .values(workshopData)
                .returning();
            ...
            const subscriptionResult = await tx.insert(subscriptions)
            ...
            const referralResult = await tx.insert(referralLinks)
                .values(referralData)
                .returning();
            ...
            const tailorsResult = await tx.insert(tailors)
                .values(tailorsData)
                .returning();
            ...
            const validatorResult = await tx.insert(validators)
            ...
            const magazinesResult = await tx.insert(magazines)
            ...
            const ordersResult = await tx.insert(orders)
                .values(ordersData)
                .returning();
            ...
            const trackingResult = await tx.insert(orderTracking)
                .values(trackingData)
                .returning();
            ...
            const assignmentLogsResult = await tx.insert(validatorAssignmentLog)
