...
├─ Fills workshop details (name, address, etc.)
├─ Chooses subscription plan (e.g., Professional plan)
├─ Pays subscription fee
└─ Gets access to full platform

STEP 2: Workshop Owner Builds Team
├─ Creates unique invitation links for tailors
├─ Sends link: "Join my workshop: https://Khayt.com/join?code=xyz123"
├─ Tailor clicks link and signs up
└─ Tailor now part of the team and can receive work

STEP 3: Boutique Shop Joins
├─ Workshop owner creates invitation for boutique shop
├─ Boutique owner clicks link and signs up
├─ Boutique can now place orders with this workshop

STEP 4: Boutique Places Order
├─ Clicks "Create Order"
├─ Specifies: "I need 5 elegant dresses, each 2 meters fabric, embroidery on chest"
├─ Sets deadline: "Need by November 30"
├─ Sets price: "Will pay $250"
└─ Submits order

STEP 5: Quality Checker Reviews Order
├─ Validator receives the order
├─ Checks if specifications are clear and doable
├─ Either APPROVES or REJECTS the order
│  ├─ If approved → moves to next step
│  └─ If rejected → tells boutique what's wrong
└─ (Boutique can fix and resubmit)

STEP 6: Work Gets Assigned to Tailors
├─ Validator looks at the order
├─ Knows each tailor's skills: "Ahmed = embroidery expert, Fatima = alterations specialist"
├─ Splits work: "Ahmed does embroidery, Fatima does general sewing"
├─ Each tailor gets specific tasks
└─ Both receive notifications on their phones

STEP 7: Tailors Complete Work
├─ Ahmed sees his task: "Embroider chest on 5 dresses"
├─ He marks it: "Started" → "In Progress" → "Completed"
├─ Fatima does the same for her part
└─ System tracks time, effort, and completion

STEP 8: Order Status Updates
├─ Status moves: PENDING → VALIDATED → TAILORING → PACKAGING → COMPLETED
├─ Boutique shop owner can see status anytime: "Ahmed finished, Fatima working on dress #3"
├─ Boutique gets push notification: "Your order is 80% complete"
└─ Tailors can see: "Your work is on track, finishing tomorrow"

STEP 9: Order Complete
├─ All tailors mark their work done
├─ Final quality check
├─ Status: COMPLETED
├─ Boutique gets notification: "Your order is ready for pickup!"
└─ Workshop prepares order for pickup

STEP 10: Order Picked Up
├─ Boutique owner comes to workshop
├─ Picks up finished clothes
├─ Both parties are happy
└─ Money automatically calculated: "Workshop made $225 (after 10% commission)"
```
...
✗ Cannot directly handle orders (delegates to validator)

Validator:
✓ Can see all orders
✓ Can approve/reject orders
✓ Can assign work to tailors
✓ Can update order status
✗ Cannot manage subscription
✗ Cannot manage other validators

Tailor:
✓ Can see only their assigned tasks
✓ Can update their task status
✓ Can see task details
✗ Cannot see other tailor's tasks
...
✗ Cannot validate or assign work
```
...
"Join my workshop as a tailor: https://Khayt.com/invite?code=abc123xyz"
                                                         ↓
Tailor clicks link → Sees form → Enters details → Automatically joins the workshop
...
Ahmed: Clicks → Signs up → Now part of team
```
...
VALIDATED (Order approved, everything is clear)
  ↓ (Validator assigns to tailors)
TAILORING (Tailors are actively working)
  ↓ (Tailors finish, package work)
...
TASK 1 → Assigned to "Zainab (Embroidery Expert)"
Purpose: Add embroidery on all 5 dress chests
Deadline: Nov 20
Skills needed: Embroidery, hand stitching

TASK 2 → Assigned to "Fatima (Sewing Specialist)"
...
Timeline:
Nov 14 ✓ Order created by shop
Nov 14 ✓ Order validated by Ahmed (quality checker)
Nov 15 ✓ Work assigned:
         - Zainab: Embroidery (started)
         - Fatima: Sewing (not started yet)
Nov 16 → Zainab finished embroidery
Nov 17 → Fatima started sewing
Nov 18 → Fatima is working (80% done)
...
kodikos-api | 🚀 Running migrations...
kodikos-api | 🌱 Seeding database...
kodikos-api | ✅ Server running on http://localhost:3232
```
...
## 🔐 Test Accounts (After Docker Setup)
...
4. **Can't see sample data?** Make sure seed job finished - check logs for "✅ Created X users"
