import { orders, orderItems, orderTracking, magazines, workshops, validators, tailors, users } from '../db/schemas/schema.js';
...
export const getWorkshopOrders = async (req, res) => {
  try {
    ...
    if (magazineId) {
      query = query.where(eq(orders.magazineId, parseInt(magazineId)));
    }
...
export const getOrderWithItems = async (req, res) => {
  try {
    ...
    const magazine = await db.select().from(magazines)
      .where(eq(magazines.id, order[0].magazineId))
      .limit(1);

    // Get workshop details
    const workshop = await db.select().from(workshops)
      .where(eq(workshops.id, order[0].workshopId))
      .limit(1);
  ...
  try {
    ...
    const order = await db.select().from(orders)
      .where(and(
        ...
        eq(orders.workshopId, validator[0].workshopId)
      ))
      .limit(1);
...
export const getOrderTimeline = async (req, res) => {
  try {
    ...
    const timeline = await db.select().from(orderTracking)
      .where(eq(orderTracking.orderId, parseInt(orderId)))
